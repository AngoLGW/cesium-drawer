...
import { GLOBE_PLOT_LAYER, CIRCLE_MEASURE } from "../config/constant";
...
class GlobeCircleDrawer {
  ...
  drawCircle(positions) {
    ...
    let distan = (dis / 1000).toFixed(3);
    let area = ((dis * dis * Math.PI) / (1000 * 1000)).toFixed(3);
    let text = "半径：" + distan + " 千米";
    text += "\n 面积：" + area + "平方千米";
  ...
  drawDynamicCircle(positions) {
    ...
    let dynamicHierarchy = new Cesium.CallbackProperty(function() {
      if (positions.length > 1) {
        let dis = _this.computeCircleRadius3D(positions);
        let distan = (dis / 1000).toFixed(3);
        let area = ((dis * dis * Math.PI) / (1000 * 1000)).toFixed(3);
        _this.entity.label.text = "半径：" + distan + " 千米";
        _this.entity.label.text += "\n 面积：" + area + "平方千米";
    ...
    let labelDynamicPositions = new Cesium.CallbackProperty(function() {
      if (positions.length > 1) {
        return positions[0];
      } else {
        return null;
    ...
    let bData = {
      ...
      shape: CIRCLE_MEASURE,
      instanceId: _this.id,
      position: labelDynamicPositions,
      label: {
        text: "",
        font: "14px Helvetica",
        fillColor: _this.textMaterial,
