...
import { GLOBE_PLOT_LAYER, POLYGON_MEASURE } from "../config/constant";
import * as Cesium from "cesium";
import { v4 as uuidv4 } from "uuid";
import * as turf from "@turf/turf";
import ee from "../../../utils/eventEmitter";

class GlobePolygonMeasure {
  constructor(viewer, options, data) {
    let { fillColor, edgeColor, textColor, cb = () => {}, plot } = options;
    this.cb = cb;
    ...
    this.color = new Color({ fillColor, edgeColor, textColor });
    this.plot = plot;
     ...
     */
    if (!data) {
      this.drawPolygonEvent();
    } else {
      this.parseData(data);
      this.drawPolylgon(this.positions);
    }
  }
   ...
   * @param {*} color
   */
  setColor({ fillColor, edgeColor, textColor }) {
    if (fillColor) {
   ...
   * @param {*} positions
   */
  drawPolylgon(positions) {
    let _this = this;
    let text = _this.getMeasureTip(positions);
   ...
   * @param {*} tempPositions
   */
  drawDynamicPolylgon(tempPositions) {
    let _this = this;
    ...
    let labelPosition = new Cesium.CallbackProperty(function() {
      if (tempPositions.length > 2) {
  ...
  drawPolygonEvent() {
    ...
    _this.drawHandler.setInputAction(function(event) {
      ...
      if (_this.positions.length === 1) {
        ...
        _this.drawDynamicPolylgon(_this.positions);
      }
  ...
  editDynamicPolygon() {
     ...
     * 绘制动态多边形
     */
    _this.drawDynamicPolylgon(_this.tempPositions);
    /**
     * 绘制动态点位
     */
  ...
  updateModifyAnchors(oid) {
    ...
    let p2 = _this.tempPositions[(oid + 1) % _this.tempPositions.length];
  ...
  getMeasureTip(pntList) {
    //计算周长时，需要首尾相连
    let temPntList = [...pntList, pntList[0]];

    let _this = this;
    let dis3d = _this.computeLineDis3d(temPntList);
    dis3d = dis3d.toFixed(3);
    let tip = "";
    if (temPntList.length > 3) {
      tip = "周长：" + dis3d + "千米";
      let area = _this.computeArea(temPntList);
  ...
  computeLineDis3d(pntList) {
    ...
    for (let i = 1; i < pntList.length; i++) {
      let p1 = pntList[i - 1];
      let p2 = pntList[i];
      let dis = _this.computeDis3d(p1, p2);
      total += dis;
    }
    return total;
  }
  ...
  computeArea(positions) {
    ...
    for (let i = 0; i < positions.length; i++) {
      let p = _this.cartesian2LonLat(positions[i]);
